...
`dags/wmes/cwms_batch_events_scheduled_jobs.py` is the Batch Events scheduled-job driver. It runs every minute, reads `/scripts/scheduled` from Batch Events using the Airflow Keycloak service client, and triggers any registry entries due at the current Airflow logical date. When office-specific scheduler clients are configured, the helper lists scheduled scripts once per office and skips only the failed office if one office token or list call fails, so other offices can still be discovered. When triggering a due script, the helper passes the script office into the token lookup so Airflow can use office-specific scheduler client credentials when configured. The registry supports hourly-at-minute entries and five-field cron expressions. Due scripts are triggered through dynamic task mapping so one office trigger does not block the others due in the same minute.
