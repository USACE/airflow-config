...
def _open_files_(col_filename: str, csv_filename: Optional[str] = None) -> tuple[TextIO, TextIO] :
    '''
    ...
    try :
        ...
        raise Exception(f"ERROR: Could not open specified text file [{col_filename}] for reading")
    if csv_filename :
        if os.path.exists(csv_filename) and not os.path.isfile(csv_filename) :
            col_file.close()
            raise Exception(f"ERROR: Specified CSV file [{csv_filename}] is not a regular file")
    else :
...
def _make_csv_record_(items: tuple) -> str :
    ...
    '''
    return ",".join(map(lambda x : f'{str(x)}' if str(x).find(',') == -1 else f'"{str(x)}"', items))
...
def make_sensor_csv(col_filename: str, csv_filename: Optional[str] = None) -> int :
    ...
    try :
        ...
        for line in col_file.readlines() :
            ...
            csv_file.write(f"{_make_csv_record_((location, pe_code, interval, a_part, b_part, f_part))}\n")
        return 0
    except Exception as e :
        sys.stderr.write(f"\n{str(e)}\n\n")
        return -1
...
def make_parameter_csv(col_filename: str, csv_filename: Optional[str] = None) -> int :
    ...
    try :
        ...
        for line in col_file.readlines() :
            ...
            csv_file.write(f"{_make_csv_record_((pe_code, c_part, unit, data_type, transform))}\n")
        return 0
    except Exception as e :
        sys.stderr.write(f"\n{str(e)}\n\n")
        return -1
