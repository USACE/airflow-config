...
# CONSTANTS
DOWNLOAD_OPERATOR_USE_CONNECTION = os.getenv(
    "DOWNLOAD_OPERATOR_USE_CONNECTION", default=None
)
DOWNLOAD_OPERATOR_USE_LAMBDA = (
    True
    if os.getenv("DOWNLOAD_OPERATOR_USE_LAMBDA", default="FALSE").upper() == "TRUE"
    else False
)
DOWNLOAD_OPERATOR_USE_LAMBDA_NAME = os.getenv(
    "DOWNLOAD_OPERATOR_USE_LAMBDA_NAME", default=None
)
...
def upload_string_s3(data, bucket, key, replace=True):
    ...
    load_str = hook.load_string(
        string_data=data, key=key, bucket_name=bucket, replace=True
    )
    return load_str
...
def s3_list_keys(bucket_name=None, prefix=None, delimiter=None):
    """ """
    hook = S3Hook(aws_conn_id=DOWNLOAD_OPERATOR_USE_CONNECTION)
...
def trigger_download_local(payload):
    ...
    return json.dumps(
        {"success": success, "url": url, "s3_bucket": bucket, "s3_key": key}
    )
