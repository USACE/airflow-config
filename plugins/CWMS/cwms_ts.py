...
class CwmsTsMixin:
    ...
    def retreive_ts_group(
        self, p_group_id, p_category_id, p_office_id, return_type="df"
    ):
        ...
        params = {"office": p_office_id, "category-id": p_category_id}
    ...
    def retrieve_ts(
        ...
        if p_start_date is not None:
            p_start_date = p_start_date.strftime("%Y-%m-%dT%H:%M:%S")

        if p_end_date is not None:
            p_end_date = p_end_date.strftime("%Y-%m-%dT%H:%M:%S")
        ...
        headerList = {"Accept": "application/json;version=2"}
        responce = queryCDA(
            self, endPoint, params, headerList, return_type, dict_key=["values"]
        )
    ...
    def write_ts(
        ...
        version_date=None,
        timezone=None,
        create_as_ltrs=False,
        store_rule=None,
        override_protection=None,
    ):
