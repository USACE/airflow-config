...
def create_dag(**kwargs):
    ...
    with DAG(
        ...
        def fetch_and_write(state_abbrev, water_usgs_parameters):
            ...
            sites = json.loads(water.get_usgs_sites_by_state(state_abbrev, conn_type))
            ...
            for i, state_site in enumerate(usgs_state_sites['value']['timeSeries']):
                ...
                if site_number in map.keys():
                    ...
                    if param_code in water_usgs_parameters and not map[site_number][param_code]:
                        insert_payload.append({'site_number':site_number, 'parameter_codes':[param_code]})
