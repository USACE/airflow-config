...
with DAG(
    ...
    # dag.doc_md = __doc__
    ##############################################
    def sync_devices_to_midas(
        aware_devices,
        metadata,
        midas_aware_param_config_instruments,
        midas_aware_instruments,
    ):

        aware_devices = json.loads(aware_devices)
        midas_aware_param_config_instruments = json.loads(
            midas_aware_param_config_instruments
        )
        ...
        for d_id, d_obj in aware_devices.items():
            ...
            if d_id not in midas_aware_param_config_instruments_dict.keys():
                ...
                #
                midas_instrument_id = midas_aware_param_config_instruments_dict[d_id][
                    "instrument_id"
                ]
                # print(f"Checking coordinates for: {d_obj['name']}")
                midas_lon = instruments_dict[midas_instrument_id]["geometry"][
                    "coordinates"
                ][0]
                midas_lat = instruments_dict[midas_instrument_id]["geometry"][
                    "coordinates"
                ][1]

                if round(float(midas_lat), 4) != round(float(lat), 4) or round(
                    float(midas_lon), 4
                ) != round(float(lon), 4):
                    ...
                    midas_project_id = midas_aware_param_config_instruments_dict[d_id][
                        "project_id"
                    ]
                    ...
                    if int(lat) != 0 and int(lon) != 0:
                        ...
                        r = h.run(
                            endpoint=endpoint, json=update_payload, headers=headers
                        )
                    else:
    ...
    (
        [get_midas_aware_param_config_task, get_midas_aware_instruments_task]
        >> flashflood_authenticate_task
        >> flashflood_get_customer
        >> get_aware_devices_task
        >> get_aware_devices_metadata_task
        >> sync_devices_to_midas_task
    )
