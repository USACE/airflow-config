...
implementation = {
    'stable': {
        ...
        'tags': ['stable', 'a2w', 'usgs', 'measurements'],
    },
    'develop': {
    ...
    def sync_usgs_measurements():
        ...
        for state in _watershed_usgs_sites.keys():
            ...
            def fetch_parse_post(sites_by_state: Dict, state: str):
                ...
                error = None
                for site, param_measures in groupby(parameter_measurements(url), key = itemgetter('site')):
                    payload = list()
                    ...
                    error = water.post_usgs_measurements(site=site, payload=payload, conn=conn_type)
