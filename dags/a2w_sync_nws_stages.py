...
def create_dag(**kwargs):
    ...
    with DAG(
        ...
        def get_water_usgs_site_numbers():
            ...
            sites = json.loads(water.get_usgs_sites_by_state(state='all', conn_type=conn_type))
        ...
        def get_water_nws_stages():
            ...
            stages = json.loads(water.get_nws_stages_by_site(site='all', conn_type=conn_type))
        ...
        ########################################
        def sync_nws_stages_to_water(state, nws_stages, water_nws_stages, water_usgs_site_numbers):
            ...
            for idx, line in enumerate(json.loads(nws_stages)):
                ...
                if line['state'].strip() == state.lower():
                    ...
                    if line['usgs id'].strip() in water_usgs_site_numbers and len(line['nws shef id'].strip()) == 5 \
                        and line['usgs id'].strip() != '' and len(line['usgs id'].strip()) >= 8 \
                        and line['usgs id'].strip().isnumeric() and line['flood stage unit'].strip() == 'FT' \
                        ...
                        if nwsid in water_nws_stages.keys():
                            ...
                            print(f'NWS site {nwsid} already in DB. Checking for changes...')
                            if water_nws_stages[nwsid]['action'] != action or water_nws_stages[nwsid]['flood'] != flood \
                                or water_nws_stages[nwsid]['moderate_flood'] != moderate_flood \
                                or water_nws_stages[nwsid]['major_flood'] != major_flood:
                                water.put_nws_stages(nwsid, payload, conn_type)
                                put_items.append(nwsid)                            
                        else:
                            # new site stages record, use POST
                            try:
                                # POST single site stages to the water API
                                water.post_nws_stages(payload, conn_type)
        ...
        get_water_usgs_sites_task = PythonOperator(
                task_id='get_water_usgs_sites',
                python_callable=get_water_usgs_site_numbers
        )

        get_nws_stages_task = PythonOperator(
                task_id='get_nws_stages',
                python_callable=get_nws_stages                
        )

        get_water_nws_stages_task = PythonOperator(
                task_id='get_water_nws_stages',
                python_callable=get_water_nws_stages                
        )
        ...
        for state in states:
            ...
            get_nws_stages_task >> get_water_usgs_sites_task >> get_water_nws_stages_task >> sync_nws_stages_task
