...
def cumulus_wpc_qpf():
    ...
    def check_new_forecast():
        ...
        if s3_forecast_datetime == wpc_forecast_datetime:
            # return False
            logging.info("WPC Forecast not updated.  Skipping this run.")
    ...
    @task()
    def download_and_notify(forecast_datetime):
        ...
        for hour in fcst_hrs[forecast_hour]:
            ...
            output = trigger_download(
                url=f"{URL_ROOT}/{PROD_SOURCE_DIR}/{filename}",
            ...
            notify_cumulus(
                json.dumps(
                    {
                        "datetime": datetime.strptime(forecast_datetime, "%Y%m%d%H")
        ...
        output = trigger_download(
            url=f"{STATUS_SOURCE}", s3_bucket=cumulus.S3_BUCKET, s3_key=STATUS_S3_KEY
        )
        return

    download_and_notify(check_new_forecast())
