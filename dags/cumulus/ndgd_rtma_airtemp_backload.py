...
def cumulus_ndgd_rtma_airtemp_backload():
    ...
    def get_src_datetime(src_filename):
        ...
        dt = datetime.strptime(dt_string, "%Y.%m.%d.%H").replace(tzinfo=timezone.utc)
    ...
    def get_new_filename(old_name):
        ...
        return f'ds.temp_{dt.strftime("%Y%m%d_%H")}.bin'
    ...
    def copy_raw_airtemp():
        ...
        # --------------------------------------
        dst_files = []
        dest_filename_prefix = f'ds.temp_{execution_date.strftime("%Y%m")}'
        key_prefix = (
...
cbrfc_dag = cumulus_ndgd_rtma_airtemp_backload()
